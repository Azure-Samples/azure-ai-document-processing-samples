...
def convert_single_pdf(
...
) -> Tuple[str, Dict[str, Image.Image], List[Page], Dict]:
    """
    Converts a single PDF file to Markdown, extracting images and figures.
    Additionally returns the OCR metadata for the document.

    Args:
        fname: The path to the PDF file.
        model_lst: A list of models to use for processing the PDF.
        max_pages: The maximum number of pages to process.
        start_page: The page number to start processing from.
        metadata: The metadata for the PDF file.
        langs: The languages to use for OCR.
        batch_multiplier: The batch multiplier for processing.
        ocr_all_pages: Whether to OCR all pages.

    Returns:
        str: The full text of the PDF file.
        Dict[str, Image.Image]: The images extracted from the PDF file.
        List[Page]: The pages of the PDF file.
        Dict: The metadata for the OCR results.
    """
    ...
    lowres_images = [render_image(
        doc[pnum], dpi=settings.SURYA_DETECTOR_DPI) for pnum in range(max_len)]
    ...
    surya_detection(lowres_images, pages, detection_model,
                    batch_multiplier=batch_multiplier)

    # OCR pages as needed
    pages, ocr_stats = run_ocr(doc, pages, langs, ocr_model,
                               batch_multiplier=batch_multiplier, ocr_all_pages=ocr_all_pages)
    ...
    surya_layout(lowres_images, pages, layout_model,
                 batch_multiplier=batch_multiplier)
    ...
    surya_order(lowres_images, pages, order_model,
                batch_multiplier=batch_multiplier)
    ...
    return full_text, doc_images, pages, out_meta
